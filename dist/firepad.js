...
firepad.FirebaseAdapter = (function (global) {
  ...
  FirebaseAdapter.prototype.markOperationAsRead = function (data, revisionId) {
    ...
    if (data.o && data.o.length > 4) {
      for (key in data.o) {
        var attributes = data.o[key];
        if (attributes[this.userId_] === 'unread' &&
          this.lastViewedRevision_ + 1 >= revisionFromId(revisionId)) {
          attributes[this.userId_] = 'read';
          data.o[key] = attributes
        }
      }
    }
...
function addNewLineAtTheEndOfText(cm) {
  var cursorLine = cm.getCursor('head').line;
  var totalLines = cm.lineCount();
  var isAtTheLastLine = totalLines - 1 === cursorLine;

  if (isAtTheLastLine) {
    // add new line and moves the cursor 1 line up
    cm.replaceSelection('\n', 'end', '+input');
