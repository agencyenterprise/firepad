  ...
  RichTextCodeMirror.prototype.newline = function () {
    ...
    if (!this.emptySelection_()) {
      ...
      if (listType && cm.getLine(cursorLine).length === 1) {
        ...
        // count total lines - check if there is a firepad method to do that
        var totalLines = document.getElementsByClassName('CodeMirror-line').length;

        // check if the user is at the last line
        var isAtTheLastLine = (totalLines - 1 === cursorLine);
        // current call to add a new line
        cm.replaceSelection('\n', 'end', '+input');
        if (isAtTheLastLine) {
          // add a new line at the nd
          cm.replaceSelection('\n', 'end', '+input');
        }
